...
var Share=function(){
	...
	Share.prototype={
		init:function(options){
			...
			this.btn.live("mouseenter",function(event){
				event.preventDefault();
				$(this).parent().find(".shareBox").css({display:"block"});
			})
			this.parent.live("mouseleave",function(event){
				event.preventDefault();
...
$(document).ready(function(){
	...
	ss.replace(/(uid|artinfo_id|style|lang|)=([^&]+)/g, function(a, p, v) {
		params[p] = v;
	});
	//评论大对象
	window.CMSCMT = {
		...
		init:function(){
			...
			var src_conf = CMSCMT.apiHost+"index.php?do=index&m=get_js_conf&artinfo_id="+ params['artinfo_id'] +"&aid=4&url="+ url;
